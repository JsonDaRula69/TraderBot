...
> `TokenStore` seam so profile-token resolution uses Infisical-backed secrets
> by default. `LocalEncryptedStore` provides the offline fallback;
> `LocalTokenStore` is preserved as a read-only migration source. Token
