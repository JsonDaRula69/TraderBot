...
`news/embeddings.py` (VoyageClient) — domain-optimized semantic analysis for financial text via Voyage AI.
...
### Data Points Storage & Retrieval

Non-article data (weather readings, economic indicators, crypto prices, sports scores, Google Trends) are stored to the `data_points` ChromaDB collection. Each DataPoint has flattened metadata: source, category, timestamp, ticker_refs, and data dict keys as `data_*` fields. DataPoints are generated by FRED, Open-Meteo, OpenWeatherMap, CoinGecko, TheSportsDB, and Google Trends sources.

The `data_points` collection is populated by two mechanisms:
1. **Daily backfill** (`traderbot-backfill-data@.timer`) — fetches Open-Meteo, FRED, and CoinGecko historical data incrementally (idempotent, skips duplicates)
2. **News ingestion** (`traderbot-news-ingest@.timer`) — fetches real-time data points alongside news articles every 30 minutes

On first deployment, `install-data-pipeline.sh` runs a one-shot 6-month backfill to seed the collection.

Data points can be queried in two ways:

1. **`traderbot data-points weather --json`** — standalone CLI for structured readings
2. **`traderbot news-context weather --include-data --json`** — combined news sentiment + data point readings in one call

For weather markets (like `KXHIGHLAX-26MAY16`), the agent should use `--include-data` to get current temperature readings alongside news sentiment, providing complete context for evaluating temperature-based markets.
...
`db/vectors.py` — persistent vector store for semantic search (ChromaDB wrapper).

#### Collections

| Collection | Purpose | Schema |
|---|---|---|
| `news` | News articles and their embeddings | `id`, `embedding`, `text`, `source`, `ticker`, `category`, `date` |
| `data_points` | Structured data readings (weather, economic, crypto, sports) | `id`, `embedding`, `source`, `category`, `date`, `data_*` fields |
| `decisions` | Trade decision logs with reasoning | `id`, `embedding`, `ticker`, `category`, `date` |
| `market_patterns` | Pattern signatures from heartbeat analysis | `id`, `embedding`, `pattern_type`, `category`, `date` |
| `news_signals` | Processed signal outputs | `id`, `embedding`, `ticker`, `category`, `date` |
...
ChromaDB collections do not have an auto-expiry TTL by default. Vectors persist until manually purged or the collection is reset.
