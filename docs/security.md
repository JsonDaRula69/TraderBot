...
Tokens are stored in the token registry (encrypted with Fernet) and resolved via `resolve_token()`. The agent receives the token via the secure launcher (not via plaintext documentation).
