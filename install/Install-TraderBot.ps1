...
function Setup-APICredentials {
    ...
    if (-not [string]::IsNullOrEmpty($kalshiSecret)) {
        $pemPath = Join-Path $CONFIG_DIR "kalshi_key.pem"
        [IO.File]::WriteAllText($pemPath, $kalshiSecret)
        Set-EnvInFile $envFile "KALSHI_PRIVATE_KEY_PATH" $pemPath
    }
    Write-Host "Kalshi credentials stored." -ForegroundColor Green
    Write-Host ""
...
function Register-TraderBotTask {
    ...
    $actionArgs = switch ($TaskType) {
        ...
        "news"     { "news ingest --continuous" }
        default    { "scan --continuous" }
    }

    $action = New-ScheduledTaskAction -Execute $venvTraderBot -Argument $actionArgs -WorkingDirectory $INSTALL_DIR
    ...
    try {
        ...
        # Set environment for the task (store profile token)
        $envFile = Join-Path $CONFIG_DIR ".env"
        Set-EnvInFile $envFile "TRADERBOT_PROFILE_TOKEN" $ProfileToken
...
function Register-HeartbeatTask {
    ...
    # Heartbeat calls the scan command which handles health check internally
    $venvTraderBot = Join-Path $INSTALL_DIR ".venv\Scripts\traderbot.exe"
    if (-not (Test-Path $venvTraderBot)) {
        return
    }
    ...
    $action = New-ScheduledTaskAction -Execute $venvTraderBot `
        -Argument "scan --heartbeat" -WorkingDirectory $INSTALL_DIR
...
function Invoke-AgentAssignment {
    ...
    Register-TraderBotTask -AgentName $agentName -ProfileToken $profileToken -Interval "HOURLY" -TaskType "agent"
    Register-HeartbeatTask -AgentName $agentName -Interval "15MINUTES"
    Write-Host ""
