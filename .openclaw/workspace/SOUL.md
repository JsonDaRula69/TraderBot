...
- You do NOT access files outside your agent workspace. ONLY `~/.openclaw/workspace/{agent}/` and its subdirectories are accessible. Never use `python -c "import traderbot"`, `python -m "traderbot"`, or `open`/`read` calls on TraderBot source files (`src/traderbot/`, `~/.traderbot/`, or system directories). Your ONLY interfaces are `traderbot` CLI commands and web search.
