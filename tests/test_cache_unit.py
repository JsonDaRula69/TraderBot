...
class TestMarketCacheTTL:
    async def test_stored_market_is_hit_before_ttl(self) -> None:
        cache = MarketDataCache.__new__(MarketDataCache)
        cache._profile = None
        cache._lock = asyncio.Lock()
        cache._market_cache = {}
        cache._orderbook_cache = {}
        cache._settlement_db_path = "/tmp/test_cache_ttl.db"
        import pathlib

        cache._settlement_db_path = pathlib.Path("/tmp/test_cache_ttlu.db")
        snap = _market()
    ...
    async def test_stored_market_expires_after_ttl(self) -> None:
        cache = MarketDataCache.__new__(MarketDataCache)
        cache._profile = None
        cache._lock = asyncio.Lock()
        cache._market_cache = {}
        ...
        cache._settlement_db_path = pathlib.Path("/tmp/test_cache_expire.db")
        snap = _market()
        now = time.monotonic()
    ...
    async def test_missing_market_returns_none(self) -> None:
        cache = MarketDataCache.__new__(MarketDataCache)
        cache._profile = None
        cache._lock = asyncio.Lock()
        cache._market_cache = {}
        ...
        cache._settlement_db_path = pathlib.Path("/tmp/test_cache_miss.db")
        result = await cache.get_market("MISSING")
        assert result is None


class TestOrderbookCacheTTL:
    async def test_stored_orderbook_is_hit_before_ttl(self) -> None:
        cache = MarketDataCache.__new__(MarketDataCache)
        cache._profile = None
        cache._lock = asyncio.Lock()
        cache._market_cache = {}
        ...
        cache._settlement_db_path = pathlib.Path("/tmp/test_ob_hit.db")
        ob = _orderbook()
        await cache.set_orderbook("TEST", ob)
        result = await cache.get_orderbook("TEST")
        assert result is not None

    async def test_expired_orderbook_returns_none(self) -> None:
        cache = MarketDataCache.__new__(MarketDataCache)
        cache._profile = None
        cache._lock = asyncio.Lock()
        cache._market_cache = {}
        ...
        cache._settlement_db_path = pathlib.Path("/tmp/test_ob_expire.db")
        ob = _orderbook()
        now = time.monotonic()
...
class TestInvalidate:
    async def test_invalidate_removes_entries(self) -> None:
        cache = MarketDataCache.__new__(MarketDataCache)
        cache._profile = None
        cache._lock = asyncio.Lock()
        cache._market_cache = {}
        ...
        cache._settlement_db_path = pathlib.Path("/tmp/test_invalidate.db")
    ...
    async def test_invalidate_all_clears_caches(self) -> None:
        cache = MarketDataCache.__new__(MarketDataCache)
        cache._profile = None
        cache._lock = asyncio.Lock()
        cache._market_cache = {}
        ...
        cache._settlement_db_path = pathlib.Path("/tmp/test_invalidate_all.db")
        await cache.set_market("A", _market("A"))
...
class TestBatchMethods:
    async def test_get_markets_batch_returns_cached(self) -> None:
        cache = MarketDataCache.__new__(MarketDataCache)
        cache._profile = None
        cache._lock = asyncio.Lock()
        cache._market_cache = {}
        ...
        cache._settlement_db_path = pathlib.Path("/tmp/test_batch.db")
        await cache.set_market("A", _market("A"))
        await cache.set_market("B", _market("B"))
    ...
    async def test_get_orderbooks_batch(self) -> None:
        cache = MarketDataCache.__new__(MarketDataCache)
        cache._profile = None
        cache._lock = asyncio.Lock()
        cache._market_cache = {}
        ...
        cache._settlement_db_path = pathlib.Path("/tmp/test_ob_batch.db")
        await cache.set_orderbook("X", _orderbook("X"))
        result = await cache.get_orderbooks_batch(["X", "Y"])
        assert "X" in result
        assert "Y" not in result


class TestSettlementPersistence:
    def test_set_and_get_settlement(self) -> None:
        import pathlib
        import tempfile

        with tempfile.TemporaryDirectory() as td:
            db_path = pathlib.Path(td) / "settlement.db"
            cache = MarketDataCache.__new__(MarketDataCache)
            cache._profile = None
            cache._lock = asyncio.Lock()
            cache._market_cache = {}
    ...
    def test_settlement_persists_across_instances(self) -> None:
        import pathlib
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            cache1 = MarketDataCache.__new__(MarketDataCache)
            cache1._profile = None
            cache1._lock = asyncio.Lock()
            cache1._market_cache = {}
            cache1._orderbook_cache = {}
            cache1._settlement_db_path = db_path
            cache1._init_settlement_table()

            settled_at = datetime(2025, 1, 2, 0, 0, 0, tzinfo=UTC)
            cache1.set_settlement("TEST-MKT", outcome=True, settled_at=settled_at)

            cache2 = MarketDataCache.__new__(MarketDataCache)
            cache2._profile = None
            cache2._lock = asyncio.Lock()
            cache2._market_cache = {}
            cache2._orderbook_cache = {}
            cache2._settlement_db_path = db_path
    ...
    def test_batch_settlement_operations(self) -> None:
        import pathlib
        import tempfile

        with tempfile.TemporaryDirectory() as td:
            db_path = pathlib.Path(td) / "settlement_batch.db"
            cache = MarketDataCache.__new__(MarketDataCache)
            cache._profile = None
            cache._lock = asyncio.Lock()
            cache._market_cache = {}
    ...
    def test_get_nonexistent_settlement_returns_none(self) -> None:
        import pathlib
        import tempfile

        with tempfile.TemporaryDirectory() as td:
            db_path = pathlib.Path(td) / "settlement_missing.db"
            cache = MarketDataCache.__new__(MarketDataCache)
            cache._profile = None
            cache._lock = asyncio.Lock()
            cache._market_cache = {}
